...
public class ExecuteBuildOffer2 extends BaseTest {

	@BeforeSuite
	public void beforeTest() {
		test = extentReports.startTest("******Build Offer 2******");
	 ...
	 */
	@Test(dataProvider = "getBuildOffer2Data", dataProviderClass = TestDataProvider.class)
	public void getBuildOffer2Test(Hashtable<String, String> data) throws Exception {
		...
		checkTestRunMode("BuildOffer2", testCaseRunMode);
		...
		Boolean isSuccess = checkLoginSuccessful(getElements("flowcast_xpath"));
		expectedResult = data.get(ADSConstants.EXPECTTED_RESULT);

		// checkTestRunMode("Buildoffer1", testCaseRunMode);

		if (isSuccess) {
			actualResult = ADSConstants.FLOWCAST_SUCCESS;

		} else {
			actualResult = ADSConstants.FLOWCAST_FAILURE;

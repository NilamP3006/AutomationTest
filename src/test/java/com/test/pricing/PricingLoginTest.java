...
public class PricingLoginTest extends BaseTest {

	@BeforeSuite
	public void beforeTest() {
		test = extentReports.startTest("******Pricing Login Test******");
	 ...
	 */
	@Test(dataProvider = "getPricingLoginTestData1", dataProviderClass = TestDataProvider.class)
	public void getLoginTest(Hashtable<String, String> data) throws Exception {
		...
		checkTestRunMode("PricingLoginTest", testCaseRunMode);
		...
		Boolean isSuccess = checkLoginSuccessful(getElements("loginsuccess_xpath"));
		Boolean isErrors = checkErrors(getElements("loginPageInLineError_xpath"));

		checkTestRunMode("PricingLoginTest", testCaseRunMode);

		if (isSuccess) {
			actualResult = getElement("loginsuccess_xpath").getText();
